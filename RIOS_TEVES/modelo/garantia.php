...
class Garantia {
    ...
    public function obtenerTodas() {
        $sql = "SELECT * FROM garantias_rios";
    ...
    public function guardar($producto, $entidad, $descripcion, $contacto, $TecCargo, $fecha) {
        $stmt = $this->conn->prepare("INSERT INTO garantias_rios (producto, entidad, descripcion, contacto, TecCargo, fecha) VALUES (?, ?, ?, ?, ?, ?)");
        if (!$stmt) {
    ...
    public function obtenerPorId($id) {
        $stmt = $this->conn->prepare("SELECT * FROM garantias_rios WHERE id = ?");
        if (!$stmt) {
    ...
    public function actualizar($id, $producto, $entidad, $descripcion, $contacto, $TecCargo, $fecha) {
        $stmt = $this->conn->prepare("UPDATE garantias_rios SET producto=?, entidad=?, descripcion=?, contacto=?, TecCargo=?, fecha=? WHERE id=?");
        if (!$stmt) {
    ...
    public function eliminar($id) {
        $stmt = $this->conn->prepare("DELETE FROM garantias_rios WHERE id=?");
        if (!$stmt) {
